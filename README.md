...
- **原项目特性** - 具体可看看原项目1.3.0的文档。这里主要说说新加的。
...
- 支持导入游戏支持.htm和.html游戏，开启游戏时会开启http服务器解决浏览器的本地文件限制
...
- **画廊** - 添加了画廊和截图功能，能看官方截图和自己截图，自己截图要使用设定的截图键，支持手柄。支持选择是否下载，但getchu源如果不选择自动下载会看不到图。自动下载的图片会按游戏分在不同文件夹，能直接打开。注意如果不同游戏用的同一个源或图片url相同，那么可能有问题。但封面没问题。支持点击图片打开图片窗口，支持切换图片，Magpie全屏。现在画廊能在游戏详情下的画廊、游戏库页和分类游戏页的画廊模式看到。
- **键位映射** - 支持简单的手柄映射和截图和触摸映射，现在所有手柄测试过了，dualsense需要usb链接，ds4可蓝牙链接，ns pro可蓝牙连需要安装BetterJoy的驱动，xinput设备直接连。支持全局手柄设置和单独游戏。注意截图键只能全局，还有启动映射需要设置好进程名，自动的话请需要等待检测完成，可在游玩配置里设置检测时间。看看到时候要不要做右摇杆映射到鼠标滚轮或鼠标移动。触摸板需要SDL3貌似太复杂先不弄了。触摸映射可供WIN平板来玩，启动游戏时显示在屏幕上。
- **评论** - 支持看批评空间，DLSITE,DMM,Bangum的评论和评分，注意这个是实时加载的，批评空间支持看详细评论。bgm失败的话试试更新一下token。
- **存档改进** - 加了能从诚也下载存档的功能，要先设定存档位置。关于存档位置，已支持搜索存档，还在完善中。
- **进程寻找改进** - 原来是要手动选进程，现在会先找exe，然后看子进程，绝大多数情况下不需要手动选进程。
- **备份和恢复** - 添加数据库导入恢复的选项，可以选全量备份的zip。修复数据库升级后不支持的问题。云备份添加支持Webdev备份。
- **视频播放** - 支持视频播放，ffmpeg.exe放在程序目录下或自己设置。注意显卡驱动要跟ffmpeg版本兼容。搜索视频来播放视频.
- **攻略板块** - 能看诚也攻略
- **分类页** - 能从多种分类中归类，如品牌，系列，收藏，类型，人设，剧本，父目录等，能有画廊模式。
- **虚拟机启动游戏** - 支持启动同目录下的虚拟机游戏，映射盘也可。支持多虚拟机管理和esx。顺便记录一下，esx解决音频问题可用这个https://vb-audio.com/Voicemeeter/vban.htm
- **选择屏幕启动游戏** - 统一游戏的输出屏幕
- **STEAM链接** - 在游戏参数填入steam链接，会用steam启动游戏
- **游戏资源页** - 能看来自getchu的每月游戏（已导入游戏有getchu id的会标示），能搜索nyaa下载rss（可修改设置，由于合规的问题，以后可能不留默认nyaa要自己填了）并添加下载到qbittorrent服务器
- **已下载文件页** - 管理下载文件夹。注意qb请设置打开为完成文件挂上.!qb后缀的功能来检查是否完成下载。能实现批量解压、安装、导入、下载存档、删除等功能。当然必须官方安装方式的无法全过程批量，但已能省下大量时间。
- **任务页** - 能看后台任务的状态和结果，能在这界面进行整理、导入等操作。包括搜刮更新游戏数据、检测游戏有效性、检测目录导入状态等。结果页能跳入游戏页、整理、导入等操作。
- **多种小改进** - 游戏库能直接看游戏时间，游戏库增加了显示模式选择支持列表，小图，大图，画廊模式。
