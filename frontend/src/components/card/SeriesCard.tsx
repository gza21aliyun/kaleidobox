...
export function SeriesCard({ series, viewMode = "default" }: SeriesCardProps) {
  ...
  const handleViewDetails = () => {
    navigate({ to: `/series/${encodeURIComponent(series.name)}` });
    series.use_count++;
    UpdateTag(series)
  ...
  const loadSeriesGames = async () => {
    ...
    try {
      const result = await GetGamesByTag(series.name);
      setGames(result || []);
      setHasLoaded(true);
    } catch (error) {
      console.error(`Failed to load games for series ${series.name}:`, error);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      >
        <div className="relative w-full overflow-hidden bg-neutral-100 dark:bg-brand-700" style={{ aspectRatio: '16/9' }}>
          {isLoading ? (
            ...
            <div className="flex items-center justify-center h-full text-neutral-400 dark:text-brand-400">
              <div className="i-mdi-format-list-bulleted-type text-5xl" />
            </div>
          )}
        </div>
        ...
        <div className="p-4">
          ...
          <p className="text-sm text-brand-500 dark:text-brand-400 mt-1">
            {games.length} {t('seriesList.labels.games')}
          </p>
        </div>
      </div>
    );
